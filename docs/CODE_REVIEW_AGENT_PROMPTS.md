...
## TIER 1: Safety and Clarity (Always Run First)

### Prompt: Logic and Correctness Check
Use for scripts handling financial data, deployment automation, or sensitive operations.
...
Use for any code you will hand off or maintain in 6 months.
...
## TIER 2: Maintainability and Patterns

### Prompt: Maintainability Review (Karpathy Principles)
Use for scripts you will iterate on (AEGIS automation, trading bots, content pipelines).
...
Use for data pipelines, automation, and integrations (especially Era Context, Supabase, Railway deployments).
...
## TIER 3: Performance and Architecture

### Prompt: Performance and Efficiency
Use for trading bots, data feeds, and content generation pipelines.

```text
Review this code for efficiency:

1. Loops and queries: Is there any O(n^2) behavior or repeated API calls that could be batched?
...
### Prompt: Architecture and Testability
...
Use for scripts handling balances, trades, account info, or crypto wallets.
...
2. Logging and audit trail: Does it log all transactions with timestamp, amount, and outcome? (You should never wonder "did that trade go through?")
...
Use for code calling Era Context, yfinance, CoinGecko, Supabase, or other external APIs.
...
### Prompt: Deployment and DevOps Safety
Use for scripts running on Hermes (VPS), cron jobs, Railway deployments, or Docker containers.
...
Use for No Fomo, RELAY framework, and any TS projects.
...
## Meta Prompts (Reviewing the Reviews)
...
## Integration with AEGIS and RELAY

For AEGIS automation scripts:

- Always run Tier 1 (Safety and Clarity) + Tier 4 (Financial Security).
- Log all financial operations and make them auditable.
